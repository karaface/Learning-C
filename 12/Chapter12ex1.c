// Example program #1 from Chapter 12 of Absolute Beginner's Guide to C, 3rd Edition
// File Chapter12ex1.c
...
main()
{
    // Setup some common integers for the program
    int planets = 8;
    int friends = 6;
    int potterBooks = 7;
    int starWars = 9;
    ...
    // This first logical statement uses the AND operator to test whether the cast of Friends and Beatles could have enough people to make a baseball team AND
    // the cast of Friends and the Avengers would have enough people to field a football team. 
    ...
    if ((friends + beatles >= baseball) && 
    ...
    else
        {
            ...
            printf("OR they can't make a Grindiron Gang with the ");
            printf("Avengers (or both)!\n");
    }

    // This second logical statement uses the OR operator to test weather the number of Star Wars movies is less than months in a year OR the number of Harry Potter books
    // is less than months in a year. If either statement is true the statements will print.
    ...
    // This final logical statement uses the NOT operator to test whether the number of baseball players on a yeam added to the number of basketball players on a team is NOT
