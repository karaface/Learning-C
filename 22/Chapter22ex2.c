// Exmaple program #2 from Chapter 22 of Absolute Beginner's Guide to C, 3rd Edition
// File Chapter22ex2.c
 ...
 * Once it knows that information, it prints the totals from all three categories from that game */
...
int main()
{
    ...
    int gameRebounds[10] = {5, 6, 1, 5, 10, 3, 0, 7, 6, 4};
    int gameAssists[10] = { 2, 9, 4, 3, 6, 1, 11, 6, 9, 10};

    int bestGame = 0; // The comparison variable for best scoring game
    int gmMark = 0; // This will mark which game is best scoring game

    int i;

    for (i = 0; i < 10; i++)
    {
        if (gameScores[i] > bestGame) // if loop will compare each game to the current best total, 
                                      // if the current score is higher it becomes the new best and the counter variable becomes the new gmMark
        {
